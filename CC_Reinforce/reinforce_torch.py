...
class PolicyGradientAgent():
    def __init__(self, lr, input_dims, gamma=0.99, n_actions=4 ,lamda = 2 ,threshold = 0.9 ,delta = 0.05) :
        self.gamma = gamma
        self.lr = lr
        self.reward_memory = []
        self.action_memory = []
    ...
    def learn(self):
        ...
        # for commulative costs (constraints)
        ...
        for g, c ,logprob in zip(G, C , self.action_memory):
            ...
            loss += (-g * logprob  + self.lamda*logprob*(float(C[0]>= self.threshold) + (self.delta)))
    ...
    #     C = np.zeros_like(self.cost_memory, dtype=np.float64)
        ...
        #     loss += (-g * logprob  - self.lamda*c*logprob*float(C[-1]<= self.threshold))
        #     #print(C[-1])
        #     # print(float(C[-1]<= self.threshold))
        ...
        # self.lamda+= self.lr*(-(1-self.delta) + float(lamda_loss <= self.threshold))
