...
from TRPO.CC_TRPO.cc_trpo import Agent as CC_TRPO_Agent
from TRPO.TRPO.trpo import Agent as TRPO_Agent

from CC_Reinforce_version_3_copy.reinforce_torch_2 import PolicyGradientAgent as CC_Reinforce_3_PolicyGradientAgent
...
# })  
    ...
    print("----------------------------------------------- REINFORCE------------------------------------")
    delta = 1.0
    agent = CC_Reinforce_3_PolicyGradientAgent(gamma=0.99, lr=0.0005, input_dims=[state_size],
    ...
    fname = 'cc_REINFORCE_'  + 'single_agent' + str(agent.lr) + '_' \
            + str(n_games) + 'games' 
    figure_file = 'plots/normal/' + fname + str(delta) + '.png'
    safety_plot_090 = 'plots/normal/' + 'probability' + str(delta)+ '.png'
    safety_plot = 'plots/normal/' + 'probability' +'.png'
    fig = 'plots/normal/' + fname + '.png'
    ...
    if not EVALUATE:
        ...
        for i in range(n_games):
            ...
            for rollout in range(num_rollouts):
                ...
                while not done:
                    ...
                    #collect samples
                    samples.append((observation.reshape(state_size), action, reward, observation_.reshape(state_size),info['cost']))
                    
                    safety += int(info['cost'])
                    # print(reward)
            ...
            safety_090.append(safety/num_rollouts)
            lamda_090.append(agent.learn(rollouts))
            scores.append(np.mean(rollout_total_rewards))

            avg_score = np.mean(scores[-100:])
            print('episode ', i,
                    'average score %.2f' % avg_score)
        label = 'CC_REINFORCE' + ' ' + str(delta)
        ...
        agent.load_model(  path="CC_Reinforce_version_3_copy/tmp/highway/state_dict_model" +  str(delta) +".pt")
        scores = []
        eval_games = 500
        counter = 0 
        time_step = 0
        ...
        print('average commulative rewards  in %d',eval_games,'is ' ,np.mean(scores))
    ...
    fname = 'cc_REINFORCE_' + 'single_agent' + str(agent.lr) + '_' \
            + str(n_games) + 'games' 
    figure_file = 'plots/normal/' + fname +  str(delta) + '.png'
    safety_plot_099 = 'plots/normal/' + 'probability' + str(delta)+ '.png'
    ...
    if not EVALUATE:
        ...
        for i in range(n_games):
            ...
            for rollout in range(num_rollouts):
                ...
                # lamda_099.append(agent.learn())
                # safety_099.append(int(agent.check_constraints_satisfaction()))
            safety_099.append(safety/num_rollouts)
            lamda_099.append(agent.learn(rollouts))
            scores.append(np.mean(rollout_total_rewards))
    ...
    fname = 'cc_REINFORCE_' + version_12 + 'single_agent' + str(agent.lr) + '_' \
            + str(n_games) + 'games' 
    figure_file = 'plots/normal/' + fname + str(delta) +  '.png'
    safety_plot_095 = 'plots/normal/' + 'probability' + str(delta)+ '.png'
    ...
    if not EVALUATE:
        ...
        for i in range(n_games):
            ...
            for rollout in range(num_rollouts):
                ...
                # lamda_099.append(agent.learn())
                # safety_099.append(int(agent.check_constraints_satisfaction()))
            safety_095.append(safety/num_rollouts)
            lamda_095.append(agent.learn(rollouts))
            scores.append(np.mean(rollout_total_rewards))
    ...
    if not EVALUATE:
        for epoch in range(n_games):
            ...
            for t in range(num_rollouts):
                ...
                rollout_total_rewards.append(rewards.sum().item())
                global_rollout += 1
            SAFETEY_CC_TRPO.append(safety/num_rollouts)
            ...
            cc_mean_total_rewards.append(mtr)

        figure_file = 'plot/CC_TRPO.png'
        label = 'CC_TRPO'
        x = [i+1 for i in range(len(cc_mean_total_rewards))]
        plot_learning_curve(cc_mean_total_rewards, x, fig,label)

        agent.save_model(path= "TRPO/CC_TRPO/tmp/2-way/state_dict_model_cc_trpo.pt")
        
    else :
        scores =[]
        counter = 0
        agent.load_model(path="TRPO/CC_TRPO/tmp/2-way/state_dict_model_cc_trpo.pt")
    
        for epoch in range(eval_games):
            rollouts = []
            rollout_total_rewards = []
            score =0
            done = False
    ...
    agent = TRPO_Agent(actor_dims, critic_dims,  
                            num_actions, n_agents, agent_idx=0,alpha=5e-4, beta=5e-4,
                            gamma =0.99 , fc1_dims=256, fc2_dims=256,chkpt_dir='tmp/trpo/')
    ...
    if not EVALUATE:
        for epoch in range(n_games):
            ...
            for t in range(num_rollouts):
                ...
                while not done:
                    ...
                    # Collect samples
                    samples.append((state.reshape(state_size), action, reward, next_state.reshape(state_size)))
            ...
            SAFETEY_TRPO.append(safety/num_rollouts)
        ...
        plot_learning_curve(mean_total_rewards, x, fig,label)

        agent.save_model(path= "TRPO/TRPO/tmp/2-way/state_dict_model_cc_trpo.pt")
        
    else :
        scores =[]
        counter = 0
        agent.load_model(path="TRPO/TRPO/tmp/2-way/state_dict_model_cc_trpo.pt")
    ...
    if not EVALUATE:
        ...
        # plotting trpo and cc_trpo together

        fig = 'plots/normal/' + 'Trpo + CC_TRPO' + '.png'
        label = 'CC_TRPO'
        x = [i+1 for i in range(len(cc_mean_total_rewards))]
        plot_learning_curve(cc_mean_total_rewards, x, fig,label)
        ...
        label ='without constraint'
        plot_learning_curve(safety_090, x, safety_plot,label)
        plt.axhline(y= 0.1, color='red', linestyle='dotted')

        plt.axhline(y= 0.01, color='green', linestyle='dotted')

        label ='CC_probability 0.01'
        plot_learning_curve(safety_099, x, safety_plot,label)

        label ='CC_probability 0.5'
        plot_learning_curve(safety_095, x, safety_plot,label)

        label = 'TRPO'
        x = [i+1 for i in range(len(SAFETEY_TRPO))]
        plot_learning_curve(SAFETEY_TRPO, x, safety_plot,label)

        label = 'CC_TRPO'
        x = [i+1 for i in range(len(SAFETEY_CC_TRPO))]
        plot_learning_curve(SAFETEY_CC_TRPO, x, safety_plot,label)
        ...
        label ='lamda with out CC'
        plot_learning_curve(lamda_090, x, lamda_plot,label)

        label ='lamda 0.01'
        plot_learning_curve(lamda_099, x, lamda_plot,label)

        label ='lamda 0.5'
        plot_learning_curve(lamda_095, x, lamda_plot,label)
