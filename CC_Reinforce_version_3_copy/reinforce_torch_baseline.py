...
class PolicyGradientAgent():
    def __init__(self, lr, input_dims, gamma=0.99, n_actions=4 ,lamda = 2 ,threshold = 0.9 ,delta = 0.1) :
        self.gamma = gamma
        self.lr = lr
        self.reward_memory = []
        self.action_memory = []
        ...
        self.value_function = ValueNetwork(self.lr, input_dims, n_actions)

    def choose_action(self, observation):
        state = T.Tensor([observation]).to(self.policy.device)
        probabilities = F.softmax(self.policy.forward(state))
        self.store_state(self.value_function.forward(state))
    ...
    def learn(self):
        ...
        for g ,c,logprob,v in zip(G,C,self.action_memory,self.state_memory):
            # loss += (-g * logprob  - self.lamda*c*logprob*float(C[-1]<= self.threshold))
            loss += -((g-v) * logprob  - self.lamda*logprob*float(C[0]> self.threshold))
        ...
        value_loss = 0
        for g ,v in zip(G,self.state_memory):
            value_loss += (g-v)**2
        ...
        if (C[0] > self.threshold):
            self.lamda  = self.lamda  + 0.1 * (1-self.delta)
        else  :
            self.lamda  = self.lamda  - 0.1 * (self.delta)
